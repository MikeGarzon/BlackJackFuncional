...
def repartir_mano_dealer(mazo, dealer, jugador):
    if valor_juego(dealer) < 21 :
        repartir_mano_dealer(mazo[1:], dealer+[mazo[0]], jugador)
    elif valor_juego(jugador) <= valor_juego(dealer) or valor_juego(jugador) < 21:
        print ("cartas Dealer: \n" +str(dealer))
...
def jugar(mazo, jugador, dealer):
    ...
    if len(mazo) > 2 and valor_juego(jugador) < 21 and input("Presione p para pedir carta, si no presione Enter: "):
        jugar(mazo[2:], jugador+[mazo[0]], dealer)
    else:
        print("Cartas Jugador: \n" + str(jugador))
        repartir_mano_dealer(mazo, dealer, jugador )

print(jugar(mezclar(baraja()),[],[]))